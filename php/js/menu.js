...
function reloadfilters(){
  ...
  if(selectGender.value != "todos_los_generos")
    book = book.filter(a => a.gender.includes(selectGender.value))

  if(inputG.value != null){
    let count = JSON.parse(localStorage.getItem('book')).length
    book = book.filter(a => a.name.includes(inputG.value.toUpperCase()))
  }
  showBooks(book)
}
